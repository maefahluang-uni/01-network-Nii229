[![Open in Codespaces](https://classroom.github.com/assets/launch-codespace-2972f46106e565e64193e422d61a12cf1da4916b45550586e14ef0a7c637dd04.svg)](https://classroom.github.com/open-in-codespaces?assignment_repo_id=20028284)
...
```
<<<<<<< HEAD
The main() method creates and starts a new thread to run the MockWebServer on port 8080. It does this by instantiating the MockWebServer object with port 8080, wrapping the server object in a Thread, calling start() on the thread to begin executing the run() method concurrently. This allows the web server to run in the background, accepting and handling HTTP client connections independently from the main application.
=======
The main() method creates and starts a new thread to run the MockWebServer on port 8080. It does this by:
1. Instantiating the MockWebServer object with port 8080.
2. Wrapping the server object in a Thread.
3. Calling start() on the thread to begin executing the run() method concurrently.
This allows the web server to run in the background, accepting and handling HTTP client connections independently from the main application.
